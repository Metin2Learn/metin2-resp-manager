...
class RespDialog(ui.ScriptWindow):
    ...
    def __LoadWindow(self):
        ...
        for i, respSlot in enumerate(self.respSlotList):
            respSlot["button"].SetEvent(ui.__mem_func__(self.__OnClickTeleportButton), i)
            respSlot["button"].ShowToolTip = lambda arg=i: ui.__mem_func__(self.__OnOverInTeleportButton(arg))
            respSlot["button"].HideToolTip = ui.__mem_func__(self.__OnOverOutTeleportButton)

        for key, wnd in self.infoWndDict.items():
            if type(wnd).__name__ == "Button":
                wnd.ShowToolTip = lambda arg=key: ui.__mem_func__(self.__OnOverInInfo(arg))
                wnd.HideToolTip = ui.__mem_func__(self.__OnOverOutInfo)
            else:
                wnd.OnMouseOverIn = lambda arg=key: ui.__mem_func__(self.__OnOverInInfo(arg))
                wnd.OnMouseOverOut = ui.__mem_func__(self.__OnOverOutInfo)
    ...
    def __OnClickDropButton(self):
        ...
        if self.__RefreshRespSlot() and self.__RefreshDropSlot():
    ...
    def __OnOverInTeleportButton(self, index):
    ...
    def __OnOverOutTeleportButton(self):
        self.__wndRenderMapWnd.Hide()
    ...
    def SetMapData(self, data, currentBossCount, maxBossCount, currentMetinCount, maxMetinCount):
        for vnum in sorted(data, key=lambda x: nonplayer.GetMonsterLevel(x) ** nonplayer.GetMonsterGrade(x)):
            ...
            button.ButtonText.SetFontName("Tahoma:13b")
